...
if(isset($_POST["category"])){
	$category_query = "SELECT * FROM categories WHERE parent_cat = 0";
	...
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			...
			";
			$r= "SELECT cat_id, cat_title FROM categories WHERE parent_cat = $cid";
			$run = mysqli_query($con,$r) or die(mysqli_error($con));
			if(mysqli_num_rows($run) > 0){
				while($w = mysqli_fetch_array($run)){
					$c1id = $w["cat_id"];
					$cat1_name = $w["cat_title"];
					echo "<li><a href='#' class='category' cid='$c1id'>$cat1_name</a></li>";
				}
			}
		}
		echo "</div>";
	}
	$c = "SELECT * FROM categories WHERE parent_cat = -1";
	$ru = mysqli_query($con,$c) or die(mysqli_error($con));
	if(mysqli_num_rows($ru) > 0){
		while($row11 = mysqli_fetch_array($ru)){
			$c2id = $row11["cat_id"];
			$cat2_name = $row11["cat_title"];
			echo "
				<div class='panel panel-info panel-heading'>
					<a href='#' class='category' cid='$c2id'>$cat2_name</a>
				</div>
			";
		}
		echo "</div>";
...
function clean($string) {
	$jkl = explode(" ",$string);
	$jkl = preg_replace('/[^A-Za-z0-9\-]/','',$jkl);
	$string = implode("\ ", $jkl);
	return $string;
}
...
if(isset($_POST["search"])){
	$Name = $_POST['search'];
	$Name = clean($Name);
	$product_query = "SELECT * FROM products WHERE product_title LIKE '%".$Name."%';";
	$run_query = mysqli_query($con,$product_query);
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			$pro_id    = $row['product_id'];
